...
function parseMeta(attr, isHead) {
  ...
  if (rxMeta.test(name) && isHead) {
    return [
      name,
      attr.content || attr[name] || attr.value || ''
    ]
  }
}

function fixUrlOneSlash(url) {
  return url.match(/^\/\b/)
    ? `/${url}`
    : url
}

function fixUrlDoubleSlash(url) {
...
function parseStandart(resObj, opts, cb) {
  ...
  let req = request.get(opts, function(error, response, body) {
    if (error) {
    ...
    .on('close', () => {
      return !isClosed && cb(new Error('Read stream was closed'))
    })
    .on('error', err => {
      resObj.error = err
      return cb(null, resObj)
...
function extractRaw(input, cb) {
  ...
  parseStandart(resObj, opts, function(err, res) {
    if (err) {
      console.log(err)
      return cb(err)
    }
...
function extract(input, cb) {
  extractRaw(input, function(err, resObj) {
    if (err) console.log(err)

    resObj.title       = clear2SpAndN(resObj.title);
    resObj.description = clear2SpAndN(resObj.description);

    resObj.images ? resObj.images = url.resolve(resObj.opts.uri, resObj.images) : resObj.images = ''
