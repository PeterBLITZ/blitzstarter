...
schema
  ...
  .get(function() {
    return this.local._plainPassword;
  });
...
schema.statics.facebookAuthorize = function(req, accessToken, refreshToken, profile, done) {
  ...
  User.findOne({ 'facebook.id': profile.id }, (err, oldUser) => {
    if (err) {
      return done(err);
    }
    if (!req.user) {
      ...
      return done(
        null,
        false,
        'Sorry, but if you want connect this account, remove old one (Facebook)'
      );
    } else {
      var user = req.user;
...
schema.statics.authorize = function(req, email, password, done) {
  var User = this;
  User.findOne({ 'local.email': email }, (err, oldUser) => {
    if (err) {
      return done(err);
    }
    if (!req.user) {
      ...
      return done(
        null,
        false,
        `Sorry, but if you want connect this account, remove old one (${email})`
      );
    } else {
      var user = req.user;
